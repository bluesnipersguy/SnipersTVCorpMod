...
SMODS.Joker({
	...
	loc_txt = {
		name = "Harold",
		text = {
			"Starts out with {X:mult,C:white}X1.5{} Mult{}",
...
SMODS.Joker({
	...
	loc_txt = {
		name = "Egg (illudethis)",
		text = {
			"{C:legendary}Legendary{} Jokers can be found in the {C:attention}shop{}.",
...
SMODS.Joker({
	...
	loc_txt = {
		name = "Landscape",
		text = {
			"For each scored {C:attention}face{} card, gain {X:mult,C:white}X0.50{} Mult{}, if discarded, gain {C:blue}20 Chips{}.",
...
SMODS.Joker({
	...
	loc_txt = {
		...
		text = {
			"For each {C:attention}${} in Joker sell value,",
...
if not SnipersTV.MathConvert then
	SnipersTV.MathConvert = 1
end
local scie = SMODS.calculate_individual_effect
function SMODS.calculate_individual_effect(effect, scored_card, key, amount, from_edition)
	if (key == "mult" or key == "mult_mod") and amount ~= 1 then
					...
					func = function()
						...
						if created_joker then
							card_eval_status_text(
								...
								nil,
								nil,
								{ message = "I cant stop winning!", colour = G.C.BLUE }
							)
						end
						return true
					end,
					...
					func = function()
						...
						if created_joker then
							card_eval_status_text(
								...
								nil,
								nil,
								{ message = "Aw dangit!", colour = G.C.BLUE }
							)
						end
						return true
					end,
...
SMODS.Joker({
	...
	calculate = function(self, card, context)
		if context.end_of_round and context.game_over and context.main_eval then
			if
				to_big(G.GAME.chips) / to_big(G.GAME.blind.chips) >= to_big(0.50)
		...
		if context.end_of_round and context.game_over == false and context.main_eval and not context.blueprint then
			if pseudorandom("group_0_822e51db") < G.GAME.probabilities.normal / card.ability.extra.odds then
				...
				card_eval_status_text(
					...
					nil,
					nil,
					{ message = "Forgiven!", colour = G.C.RED }
				)
				SMODS.calculate_effect({
					func = function()
